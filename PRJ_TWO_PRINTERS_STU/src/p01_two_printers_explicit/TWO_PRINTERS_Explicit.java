...
class PrintingMonitor {
	...
	/* COMPLETE */
	private Condition colourAvaiblable = lock.newCondition();
	private Condition BWAvailable = lock.newCondition();
	...
	public PrinterCode requestColourPrinter(int procID) {
		...
		/* COMPLETE */
		while (!colourFree){
			colourAvaiblable.awaitUninterruptibly();
		}
		colourFree = false;
